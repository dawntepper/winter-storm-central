/**
 * Alert-to-Subscriber Matching Logic
 *
 * Matches NWS weather alerts to Kit subscribers based on
 * their zip code custom field. Uses zip code prefix → state
 * mapping for efficient matching.
 */

// ============================================
// ZIP CODE PREFIX → STATE MAPPING
// First 3 digits of US zip codes map to states
...
/**
 * Extract all affected state codes from a parsed NWS alert
 * Uses UGC codes from the alert geocode property
 */
function getAffectedStates(alert) {
  const states = new Set();

  // Primary: use the state field from parsed alert
  if (alert.state) {
    states.add(alert.state);
  }

  // Also extract from areaDesc which may mention multiple states
  // Format: "County1, ST; County2, ST; ..."
  if (alert.areaDesc) {
    const stateMatches = alert.areaDesc.match(/,\s*([A-Z]{2})/g);
    if (stateMatches) {
      for (const match of stateMatches) {
        const state = match.replace(/,\s*/, '');
        if (state.length === 2) {
          states.add(state);
        }
      }
...
/**
 * Group alerts by affected state
 * Returns: { "NY": [alert1, alert2], "PA": [alert3] }
 */
function groupAlertsByState(alerts) {
  const grouped = {};

  for (const alert of alerts) {
    const states = getAffectedStates(alert);
...
/**
 * Group Kit subscribers by state based on their zip code custom field
 * Returns: { "NY": [sub1, sub2], "TX": [sub3] }
 */
function groupSubscribersByState(subscribers, zipFieldKey = 'zip_code') {
  const grouped = {};

  for (const subscriber of subscribers) {
    const zip = subscriber.fields?.[zipFieldKey];
    if (!zip) continue;
...
/**
 * Match alerts to subscribers
 * Returns an array of { state, alerts, subscriberCount } objects
 * for states that have both alerts AND subscribers
 */
function matchAlertsToSubscribers(alerts, subscribersByState) {
  const alertsByState = groupAlertsByState(alerts);
  const matches = [];

  for (const [state, stateAlerts] of Object.entries(alertsByState)) {
    const subscribers = subscribersByState[state];
  ...
  // Sort by subscriber count descending (process largest audiences first)
  matches.sort((a, b) => b.subscriberCount - a.subscriberCount);
